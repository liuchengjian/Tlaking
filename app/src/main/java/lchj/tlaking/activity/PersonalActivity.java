...
import com.dqjq.factory.presenter.contact.PersonalContract;
import com.dqjq.factory.presenter.contact.PersonalPresenter;
...
public class PersonalActivity extends PresenterToolbarActivity<PersonalContract.Presenter> implements PersonalContract.View {
    ...
    @OnClick(R.id.btn_say_hellos)
    void onSayHelloClick() {
        // 发起聊天的点击
        User user = mPresenter.getUserPersonal();
        if (user == null)
            return;
        startAct(MessageActivity.class,MessageActivity.KEY_RECEIVER_ID,false);
